...
**Issue**: GitHub Actions jobs remained in "Queued" state for >10 minutes despite having active runner instances.
**Cause 1 (Label Mismatch)**: The workflow YAML (`ci_report.yml`) requested `runs-on: [self-hosted, linux, gcp-micro]`, but the runners were deployed with the label `gcp-runner` in `gcp-startup-script.sh`.
**Cause 2 (Missing Infrastructure)**: At one point, `gcloud compute instances list` returned 0 results, indicating the Managed Instance Group (MIG) and instances had been deleted (likely by a cleanup script or manual intervention) but not redeployed.
**Solution**:
1.  **Fix Labels**: Updated `ci_report.yml` to use `gcp-runner` to match the startup script configuration.
2.  **Redeploy**: Re-ran `./scripts/gcp-runner-deploy.sh` to recreate the missing Instance Template and MIG.
